...
namespace Advent02
{
    class Program
    {
        ...
        private static int Part1(){
            int count = 0;
            var lines = inputs.Trim().Split('\n');
            foreach(var line in lines){
                (int[] bornes, char letter, string password) = ReadLine(line);
                int nbLetter = password.Count(v => v == letter);                  
                if(nbLetter >= bornes[0] && nbLetter <= bornes[1])
                {
                   count++;
                }
            }            
        ...
        private static (int[] bornes, char letter, string password) ReadLine(string line){
            var fragments = line.Split(' ');
            var bornes = fragments[0].Split('-').Select(c => int.Parse(c)).ToArray();
            char letter = fragments[1][0];
            var password = fragments[2];
            return (bornes,letter,password);
        }
        ...
        private static int Part2(){
            ...
            foreach(var line in lines){
                (int[] bornes, char letter, string password) = ReadLine(line);
                int NbCorrectOccurences = bornes.Count(c => password[c-1] == letter);
                                
                if(NbCorrectOccurences == 1)
                {
                   count++;
                }
            }            
        ...
        private const string inputs =@"3-4 j: tjjj
